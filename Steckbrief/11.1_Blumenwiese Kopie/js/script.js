...
(function (L11_1_Blumenwiese) {
    ...
    function createBee() {
        for (let i = 0; i < 10; i++) {
            ...
            let differenceVelocity = L11_1_Blumenwiese.Vector.getDifference(beePosition, flowerArray[i].position);
            let length = differenceVelocity.length();
            differenceVelocity.scale(1 / length);
            let bee = new L11_1_Blumenwiese.Bee(beePosition, differenceVelocity, flowerArray[i]);
            bee.draw();
