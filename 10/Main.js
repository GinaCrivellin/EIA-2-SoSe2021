...
(function (L10_AsteroidsInheritance) {
    ...
    function getAsteroidHit(_hotspot) {
        for (let moveable of moveables) {
            if (moveable instanceof L10_AsteroidsInheritance.Asteroids && moveable.isHit(_hotspot))
                return moveable;
        }
        return null;
    }
    function breakAsteroid(_asteroid) {
        if (_asteroid.size > 0.3) {
            for (let i = 0; i < 2; i++) {
                let fragment = new L10_AsteroidsInheritance.Asteroids(_asteroid.size / 2, _asteroid.position);
    ...
    function createAsteroids(_nAsteroids) {
        ...
        for (let i = 0; i < _nAsteroids; i++) {
            let asteroid = new L10_AsteroidsInheritance.Asteroids(1.0);
            moveables.push(asteroid);
        }
    }
    function createUfo() {
        console.log("Ufo created");
        let ufo = new L10_AsteroidsInheritance.Ufo(new L10_AsteroidsInheritance.Vector(100, 100));
        moveables.push(ufo);
    }
    function update() {
        // console.log("Update");
