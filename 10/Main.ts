namespace L10_AsteroidsInheritance {
    ...
    function shootLaser(_event: MouseEvent): void {
        ...
        let asteroidHit: Asteroids | null = getAsteroidHit(hotspot);
        console.log(asteroidHit);
        if (asteroidHit)
            breakAsteroid(asteroidHit);
    }

    function getAsteroidHit(_hotspot: Vector): Asteroids | null {
        for (let moveable of moveables) {
            if (moveable instanceof Asteroids && moveable.isHit(_hotspot))
                return moveable;
        }
        return null;
    }

    function breakAsteroid(_asteroid: Asteroids): void {
        if (_asteroid.size > 0.3) {
            for (let i: number = 0; i < 2; i++) {
                let fragment: Asteroids = new Asteroids(_asteroid.size / 2, _asteroid.position);
    ...
    function createAsteroids(_nAsteroids: number): void {
        ...
        for (let i: number = 0; i < _nAsteroids; i++) {
            let asteroid: Asteroids = new Asteroids(1.0);
    ...
    function createUfo (): void {
        ...
        let ufo: Ufo = new Ufo(new Vector(100, 100));
        moveables.push(ufo);
    }
